...
tf_buffer = tf2_ros.BufferCore(rospy.Duration(100.0))  # tf buffer length
...
ts3_geom=ts2.pose.pose
print("ts3_geom type is: \n",type(ts3_geom))
...
res = pm.toTf(pm.fromMsg(ts3_geom))
print("res is: \n",res)
#ts3_tf=TransformStamped()



#tf2_ros.convert(ts3_geom,"TransformStamped")

#ts3_tf=tf2_geometry_msgs.from_msg_msg(ts3_geom)

#print('ts3')
#print('ts3_tf type is: \n',type(ts3_tf))
print('ts3_tf type is: \n',type(res))

# create buffer core class from ts1 with the buffercore settransform
#tf_buffer.lookup_transform(ts1, "default_authority") # bool tf2::BufferCore::setTransform(const geometry_msgs::TransformStamped & transform, const std::string & authority,bool 	is_static = false)
