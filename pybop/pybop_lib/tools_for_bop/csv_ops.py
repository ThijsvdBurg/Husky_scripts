...
def write_csv_pose_analysis(evaluation_result_path, filename, obj_id, scene_id_, img_id_, gt_stamps_, pred_stamps_, img_stamps_, r_, t_, scores_, azi_, alti_, bbox_=None, bbox_stamps_=None):
    filename = os.path.join(evaluation_result_path, filename + '.csv')
    f = open(filename, "w")
    f.write("scene_id,obj_id,im_id,gt_stamp,pred_stamp,img_stamp,score,R,t,azimuth,altitude,bbox,bbox_stamp,time\n")

    for scene_id, img_id, gt_stamp, pred_stamp, image_stamp, r, t, score, azimuth, altitude, bbox, bbox_stamp in zip(scene_id_, img_id_, gt_stamps_, pred_stamps_, img_stamps_, r_, t_, scores_, azi_, alti_, bbox_, bbox_stamps_):
        if score == -1:
            continue
        r11 = r[0][0]
        r12 = r[0][1]
